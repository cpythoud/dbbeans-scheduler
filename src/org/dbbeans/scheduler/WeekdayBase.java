...
public abstract class WeekdayBase extends DbBean implements DbBeanInterface {
	private long id;
	private String code = "";

	protected static final String DATABASE_TABLE_NAME = "schdlr_weekdays";
	protected static final String DATABASE_FIELD_LIST = "schdlr_weekdays.id, schdlr_weekdays.code";

	protected final BeanInternals weekdayInternals = new BeanInternals("org-dbbeans-scheduler-Weekday");
	protected static final WeekdayParameters WEEKDAY_PARAMETERS = new WeekdayParameters();

	public WeekdayBase() { }

	public WeekdayBase(final long id) {
		setId(id);
	}

	public WeekdayBase(final WeekdayBase weekdayModel) {
		id = 0;
		code = weekdayModel.code;
	}

	protected WeekdayBase(final long id, final String code) {
		this.id = id;
		this.code = code;
	}

	protected WeekdayBase(final ResultSet rs) throws SQLException {
	...
	public void setId(final long id) {
		...
		dbAccess.processQuery("SELECT code FROM schdlr_weekdays WHERE id=?", dataFromDBQuery);
	...
	public boolean equals(final Object object) {
		...
		if (object instanceof Weekday)
			return ((Weekday) object).getId() == id;
	...
	public String getCodeLabel() {
		return weekdayInternals.getLabel("code");
	}
	...
	public long updateDB(final DBTransaction transaction) {
		if (WEEKDAY_PARAMETERS.USE_CACHE)
	...
	protected boolean isDataOK(final DBTransaction transaction) {
		weekdayInternals.clearErrorMessages();
		boolean ok = true;
	...
	protected boolean checkDataForCode() {
		if (isCodeEmpty()) {
			if (isCodeRequired()) {
				weekdayInternals.addErrorMessage(id, "code", getCodeLabel(), getCodeEmptyErrorMessage());
				return false;
			}
		} else if (!isCodeOK()) {
			weekdayInternals.addErrorMessage(id, "code", getCodeLabel(), getCodeBadFormatErrorMessage());
			return false;
		} else if (isCodeToBeUnique() && !isCodeUnique()) {
			weekdayInternals.addErrorMessage(id, "code", getCodeLabel(), getCodeNotUniqueErrorMessage());
			return false;
		}
	...
	public String getCodeEmptyErrorMessage() {
		return weekdayInternals.getRequiredErrorMessage("code");
	}
	...
	public String getCodeBadFormatErrorMessage() {
		return weekdayInternals.getBadFormatErrorMessage("code");
	}

	public boolean isCodeUnique() {
		return !dbAccess.processQuery("SELECT id FROM schdlr_weekdays WHERE code=? AND id <> ?", new BooleanCheckQuery() {
			@Override
			public void setupPreparedStatement(final PreparedStatement stat) throws SQLException {
	...
	public String getCodeNotUniqueErrorMessage() {
		return weekdayInternals.getNotUniqueErrorMessage("code");
	}

	@Override
	public List<ErrorMessage> getErrorMessages() {
		return weekdayInternals.getErrorMessages();
	}
	...
	public void delete() {
		...
		if (WEEKDAY_PARAMETERS.USE_CACHE)
			WEEKDAY_PARAMETERS.CACHE_SET.delete(id);
	...
	public void delete(final DBTransaction transaction) {
		...
		transaction.addUpdate("DELETE FROM schdlr_weekdays WHERE id=?", new DBQuerySetup() {
				@Override
				public void setupPreparedStatement(final PreparedStatement stat) throws SQLException {
					stat.setLong(1, id);
				}
	...
	private long createRecord(final DBTransaction transaction) {
		...
		final long id = transaction.addRecordCreation("INSERT INTO schdlr_weekdays (code) VALUES (?)", new RecordCreationSetup());
	...
	private void updateRecord(final DBTransaction transaction) {
		...
		transaction.addUpdate("UPDATE schdlr_weekdays SET code=? WHERE id=?", new RecordUpdateSetup());
	...
	private void updateCaching() {
		if (WEEKDAY_PARAMETERS.USE_CACHE)
			WEEKDAY_PARAMETERS.CACHE_SET.submit((Weekday) this);
	}

	public static List<Weekday> getAll() {
		return getAll(WEEKDAY_PARAMETERS.getOrderByFields());
	}

	protected static List<Weekday> getAll(final String orderBy) {
		return getSelection(null, orderBy, null);
	}

	private static class GetSelectionQueryProcess implements DBQueryRetrieveData<List<Weekday>> {
		@Override
		public List<Weekday> processResultSet(final ResultSet rs) throws SQLException {
			final List<Weekday> list = new ArrayList<Weekday>();

			while (rs.next())
				list.add(new Weekday(rs.getLong(1), rs.getString(2)));
	...
	protected static List<Weekday> getSelection(final String whereClause) {
		return getSelection(whereClause, null);
	}

	protected static List<Weekday> getSelection(final String whereClause, final DBQuerySetup setup) {
		return getSelection(whereClause, WEEKDAY_PARAMETERS.getOrderByFields(), setup);
	}

	protected static List<Weekday> getSelection(final String whereClause, final String orderBy, final DBQuerySetup setup) {
		if (whereClause == null && setup != null)
			throw new IllegalArgumentException("Cannot accept setup code without a WHERE clause.");

		final StringBuilder query = new StringBuilder();
		query.append("SELECT id, code FROM schdlr_weekdays");
		if (whereClause != null)
	...
	protected static long getSelectionCount(final String whereClause, final DBQuerySetup setup) {
		final String query = "SELECT COUNT(id) FROM schdlr_weekdays WHERE " + whereClause;
	...
	protected static List<IdNamePair> getIdNamePairs(final String whereClause, final List<String> dataFields, final List<String> orderingFields) {
		return DBQueries.getIdNamePairs(db, "schdlr_weekdays", whereClause, dataFields, orderingFields);
	}

	public static long getCount() {
		return DBQueries.getLongCount(db, "schdlr_weekdays");
	}

	public static boolean isIdOK(final long id) {
		return DBQueries.isIdOK(db, "schdlr_weekdays", id);
	}

	public static boolean isIdOK(final long id, final DBTransaction transaction) {
		return DBQueries.isIdOK(transaction, "schdlr_weekdays", id);
	}
	...
	public static String getHumanReadableTitle(final long id) {
		...
		return DBQueries.getHumanReadableTitle(db, "schdlr_weekdays", id, WEEKDAY_PARAMETERS.getNamingFields());
	}

	public static List<Weekday> getList(final ResultSet rs) throws SQLException {
		final List<Weekday> list = new ArrayList<Weekday>();

		while (rs.next())
			list.add(new Weekday(rs));
	...
	public void setLocale(final Locale locale) {
		weekdayInternals.setLocale(locale);
	}
