...
public abstract class WeekdayPlaceInMonthBase extends DbBean implements DbBeanInterface {
	private long id;
	private String code = "";

	protected static final String DATABASE_TABLE_NAME = "schdlr_weekday_places_in_month";
	protected static final String DATABASE_FIELD_LIST = "schdlr_weekday_places_in_month.id, schdlr_weekday_places_in_month.code";

	protected final BeanInternals weekdayPlaceInMonthInternals = new BeanInternals("org-dbbeans-scheduler-WeekdayPlaceInMonth");
	protected static final WeekdayPlaceInMonthParameters WEEKDAY_PLACE_IN_MONTH_PARAMETERS = new WeekdayPlaceInMonthParameters();

	public WeekdayPlaceInMonthBase() { }

	public WeekdayPlaceInMonthBase(final long id) {
		setId(id);
	}

	public WeekdayPlaceInMonthBase(final WeekdayPlaceInMonthBase weekdayPlaceInMonthModel) {
		id = 0;
		code = weekdayPlaceInMonthModel.code;
	}

	protected WeekdayPlaceInMonthBase(final long id, final String code) {
		this.id = id;
		this.code = code;
	}

	protected WeekdayPlaceInMonthBase(final ResultSet rs) throws SQLException {
	...
	public void setId(final long id) {
		...
		dbAccess.processQuery("SELECT code FROM schdlr_weekday_places_in_month WHERE id=?", dataFromDBQuery);
	...
	public boolean equals(final Object object) {
		...
		if (object instanceof WeekdayPlaceInMonth)
			return ((WeekdayPlaceInMonth) object).getId() == id;
	...
	public String getCodeLabel() {
		return weekdayPlaceInMonthInternals.getLabel("code");
	}
	...
	public long updateDB(final DBTransaction transaction) {
		if (WEEKDAY_PLACE_IN_MONTH_PARAMETERS.USE_CACHE)
	...
	protected boolean isDataOK(final DBTransaction transaction) {
		weekdayPlaceInMonthInternals.clearErrorMessages();
		boolean ok = true;
	...
	protected boolean checkDataForCode() {
		if (isCodeEmpty()) {
			if (isCodeRequired()) {
				weekdayPlaceInMonthInternals.addErrorMessage(id, "code", getCodeLabel(), getCodeEmptyErrorMessage());
				return false;
			}
		} else if (!isCodeOK()) {
			weekdayPlaceInMonthInternals.addErrorMessage(id, "code", getCodeLabel(), getCodeBadFormatErrorMessage());
			return false;
		} else if (isCodeToBeUnique() && !isCodeUnique()) {
			weekdayPlaceInMonthInternals.addErrorMessage(id, "code", getCodeLabel(), getCodeNotUniqueErrorMessage());
			return false;
		}
	...
	public String getCodeEmptyErrorMessage() {
		return weekdayPlaceInMonthInternals.getRequiredErrorMessage("code");
	}
	...
	public String getCodeBadFormatErrorMessage() {
		return weekdayPlaceInMonthInternals.getBadFormatErrorMessage("code");
	}

	public boolean isCodeUnique() {
		return !dbAccess.processQuery("SELECT id FROM schdlr_weekday_places_in_month WHERE code=? AND id <> ?", new BooleanCheckQuery() {
			@Override
			public void setupPreparedStatement(final PreparedStatement stat) throws SQLException {
	...
	public String getCodeNotUniqueErrorMessage() {
		return weekdayPlaceInMonthInternals.getNotUniqueErrorMessage("code");
	}

	@Override
	public List<ErrorMessage> getErrorMessages() {
		return weekdayPlaceInMonthInternals.getErrorMessages();
	}
	...
	public void delete() {
		...
		if (WEEKDAY_PLACE_IN_MONTH_PARAMETERS.USE_CACHE)
			WEEKDAY_PLACE_IN_MONTH_PARAMETERS.CACHE_SET.delete(id);
	...
	public void delete(final DBTransaction transaction) {
		...
		transaction.addUpdate("DELETE FROM schdlr_weekday_places_in_month WHERE id=?", new DBQuerySetup() {
				@Override
				public void setupPreparedStatement(final PreparedStatement stat) throws SQLException {
					stat.setLong(1, id);
				}
	...
	private long createRecord(final DBTransaction transaction) {
		...
		final long id = transaction.addRecordCreation("INSERT INTO schdlr_weekday_places_in_month (code) VALUES (?)", new RecordCreationSetup());
	...
	private void updateRecord(final DBTransaction transaction) {
		...
		transaction.addUpdate("UPDATE schdlr_weekday_places_in_month SET code=? WHERE id=?", new RecordUpdateSetup());
	...
	private void updateCaching() {
		if (WEEKDAY_PLACE_IN_MONTH_PARAMETERS.USE_CACHE)
			WEEKDAY_PLACE_IN_MONTH_PARAMETERS.CACHE_SET.submit((WeekdayPlaceInMonth) this);
	}

	public static List<WeekdayPlaceInMonth> getAll() {
		return getAll(WEEKDAY_PLACE_IN_MONTH_PARAMETERS.getOrderByFields());
	}

	protected static List<WeekdayPlaceInMonth> getAll(final String orderBy) {
		return getSelection(null, orderBy, null);
	}

	private static class GetSelectionQueryProcess implements DBQueryRetrieveData<List<WeekdayPlaceInMonth>> {
		@Override
		public List<WeekdayPlaceInMonth> processResultSet(final ResultSet rs) throws SQLException {
			final List<WeekdayPlaceInMonth> list = new ArrayList<WeekdayPlaceInMonth>();

			while (rs.next())
				list.add(new WeekdayPlaceInMonth(rs.getLong(1), rs.getString(2)));
	...
	protected static List<WeekdayPlaceInMonth> getSelection(final String whereClause) {
		return getSelection(whereClause, null);
	}

	protected static List<WeekdayPlaceInMonth> getSelection(final String whereClause, final DBQuerySetup setup) {
		return getSelection(whereClause, WEEKDAY_PLACE_IN_MONTH_PARAMETERS.getOrderByFields(), setup);
	}

	protected static List<WeekdayPlaceInMonth> getSelection(final String whereClause, final String orderBy, final DBQuerySetup setup) {
		if (whereClause == null && setup != null)
			throw new IllegalArgumentException("Cannot accept setup code without a WHERE clause.");

		final StringBuilder query = new StringBuilder();
		query.append("SELECT id, code FROM schdlr_weekday_places_in_month");
		if (whereClause != null)
	...
	protected static long getSelectionCount(final String whereClause, final DBQuerySetup setup) {
		final String query = "SELECT COUNT(id) FROM schdlr_weekday_places_in_month WHERE " + whereClause;
	...
	protected static List<IdNamePair> getIdNamePairs(final String whereClause, final List<String> dataFields, final List<String> orderingFields) {
		return DBQueries.getIdNamePairs(db, "schdlr_weekday_places_in_month", whereClause, dataFields, orderingFields);
	}

	public static long getCount() {
		return DBQueries.getLongCount(db, "schdlr_weekday_places_in_month");
	}

	public static boolean isIdOK(final long id) {
		return DBQueries.isIdOK(db, "schdlr_weekday_places_in_month", id);
	}

	public static boolean isIdOK(final long id, final DBTransaction transaction) {
		return DBQueries.isIdOK(transaction, "schdlr_weekday_places_in_month", id);
	}
	...
	public static String getHumanReadableTitle(final long id) {
		...
		return DBQueries.getHumanReadableTitle(db, "schdlr_weekday_places_in_month", id, WEEKDAY_PLACE_IN_MONTH_PARAMETERS.getNamingFields());
	}

	public static List<WeekdayPlaceInMonth> getList(final ResultSet rs) throws SQLException {
		final List<WeekdayPlaceInMonth> list = new ArrayList<WeekdayPlaceInMonth>();

		while (rs.next())
			list.add(new WeekdayPlaceInMonth(rs));
	...
	public void setLocale(final Locale locale) {
		weekdayPlaceInMonthInternals.setLocale(locale);
	}
